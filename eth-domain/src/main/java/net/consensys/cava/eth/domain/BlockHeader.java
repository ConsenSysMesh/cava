...
public final class BlockHeader {
  ...
  static BlockHeader readFrom(RLPReader reader) {
    return new BlockHeader(Optional.of(reader.readValue()).map(bytes -> {
      if (bytes.isEmpty()) {
        return null;
      } else {
  ...
  public BlockHeader(
      ...
      Hash mixHash,
      Bytes nonce) {
    checkNotNull(parentHash);
    checkNotNull(ommersHash);
    checkNotNull(coinbase);
    checkNotNull(stateRoot);
    checkNotNull(transactionsRoot);
    checkNotNull(receiptsRoot);
    checkNotNull(logsBloom);
  ...
  public boolean equals(Object o) {
    ...
    return Objects.equal(parentHash, that.parentHash)
        && Objects.equal(ommersHash, that.ommersHash)
        && Objects.equal(coinbase, that.coinbase)
        && Objects.equal(stateRoot, that.stateRoot)
        && Objects.equal(transactionsRoot, that.transactionsRoot)
        && Objects.equal(receiptsRoot, that.receiptsRoot)
        && Objects.equal(logsBloom, that.logsBloom)
        && Objects.equal(difficulty, that.difficulty)
        && Objects.equal(number, that.number)
        && Objects.equal(gasLimit, that.gasLimit)
        && Objects.equal(gasUsed, that.gasUsed)
        && Objects.equal(timestamp, that.timestamp)
        && Objects.equal(extraData, that.extraData)
        && Objects.equal(mixHash, that.mixHash)
   ...
   */
  public Optional<Hash> parentHash() {
    return parentHash;
  }
  ...
  void writeTo(RLPWriter writer) {
    writer.writeValue(parentHash.map(Hash::toBytes).orElse(Bytes.EMPTY));
